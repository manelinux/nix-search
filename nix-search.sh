...
search_user() {
  ...
  for user_home in "${homes[@]}"; do
    ...
    # User nix profile — covers both nix-env and user-level installs
    local profile="$user_home/.local/state/nix/profiles/profile"
    if [[ ! -e "$profile" ]]; then continue; fi

    local all_pkgs
    # Use nix-env -q for current user (shows only explicitly installed packages)
    # Fall back to pkgs_from_store for other users
...
show_duplicates() {
  ...
  local dupes
  # Deduplicate within each scope first, then find packages present in more than one scope
