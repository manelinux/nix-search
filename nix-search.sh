...
# --- Print packages in a 2-column table ---
print_table() {
  local input="$1"
  local cols=2
  local col_width=36
...
# --- Temporary directory for duplicates ---
...
# --- Helper: get packages from nix store path (modern) ---
pkgs_from_store() {
  local path="$1"
  nix path-info --recursive "$path" 2>/dev/null \
    | xargs -I{} basename {} \
    | sed 's/^[a-z0-9]\{32\}-//' \
    | sort -u
}

# --- Helper: get packages from config.nix using nix eval ---
get_packages_from_nix_config() {
  local attr_path="$1"
  if [[ -z "$attr_path" ]]; then
    return
  fi
  nix eval --file "$NIXOS_CONFIG" --apply "
    let
      config = (import $NIXOS_CONFIG {});
      pkgName = pkg:
        if builtins.isString pkg then
          pkg
        else if builtins.hasAttr \"name\" pkg then
          (builtins.parseDrvName pkg.name).name
        else if builtins.hasAttr \"pname\" pkg then
          pkg.pname
        else
          null;
      packages = config.${attr_path} or [];
      names = builtins.filter (n: n != null) (builtins.map pkgName packages);
    in
...
# --- Scope: config (NEW: uses nix eval) ---
search_config() {
  local only_dupes="${1:-false}"
  ...
  local sys_pkgs
  sys_pkgs=$(get_packages_from_nix_config "environment.systemPackages")
  save_for_dupes "config_system" "$sys_pkgs"

  local user_names
  user_names=$(grep -oP 'users\.users\.\K[a-zA-Z0-9_-]+' "$NIXOS_CONFIG" | sort -u)

  if [[ "$only_dupes" == true ]]; then
    while IFS= read -r username; do
      local user_pkgs
      user_pkgs=$(get_packages_from_nix_config "users.users.${username}.packages")
      save_for_dupes "config_user_${username}" "$user_pkgs"
    done <<< "$user_names"
    return
  fi
  ...
  while IFS= read -r username; do
    ...
    header "👤 CONFIGURATION.NIX — users.users.${username}.packages"
