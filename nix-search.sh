...
# --- Valid scopes ---
VALID_SCOPES=("user" "global" "home-manager" "all")
...
validate_scope() {
  ...
  echo -e "Valid options: ${CYAN}user, global, home-manager, all${RESET}"
  exit 1
}
...
expand_scopes() {
  ...
  for scope in "${SCOPES[@]}"; do
    if [[ "$scope" == "all" ]]; then
      expanded+=("user" "global" "home-manager")
    else
      expanded+=("$scope")
    fi
  done
...
# --- Scope: user (nix-env + user profile) ---
# Covers both nix-env installs and user-level nix profiles
search_user() {
  ...
  for user_home in "${homes[@]}"; do
    ...
    # Collect packages from nix-env and user profile, deduplicated
    local all_pkgs=""

    # nix-env packages (only for current user)
    if [[ "$user_home" == "$HOME" ]]; then
      local nix_env_pkgs
      nix_env_pkgs=$(nix-env -q --installed 2>/dev/null)
      all_pkgs="$nix_env_pkgs"
    fi

    # User nix profile (~/.local/state/nix/profiles/profile)
    local profile="$user_home/.local/state/nix/profiles/profile"
    if [[ -e "$profile" ]]; then
      local profile_pkgs
      profile_pkgs=$(pkgs_from_store "$profile")
      all_pkgs=$(printf '%s\n%s' "$all_pkgs" "$profile_pkgs" | sort -u | grep -v '^$')
    fi

    if [[ -z "$all_pkgs" ]]; then continue; fi

    found_any=true
    save_for_dupes "user_$user" "$all_pkgs"
    [[ "$only_dupes" == true ]] && continue

    header "📦 USER PACKAGES — $user"
    local result
...
search_home_manager() {
  ...
  for user_home in "${homes[@]}"; do
    ...
    local hm_profile=""
    # Detect real Home Manager installations only
